    ...
    >
      ...
      >
        <div style={{
          ...
          padding: '20px 24px 16px', borderBottom: '1px solid #F5EDE0',
        }}>
          <h3 style={{ margin: 0, fontFamily: 'Georgia, serif', color: '#B8860B', fontSize: '18px' }}>{title}</h3>
          <button onClick={onClose} style={{
            background: 'none', border: 'none', cursor: 'pointer',
            color: '#666', display: 'flex', padding: '4px',
            borderRadius: '4px',
          }}>
            <X size={20} />
          </button>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} width="400px">
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
        <button onClick={onClose} style={{
          padding: '8px 20px', border: '1px solid #B8860B', borderRadius: '6px',
          background: 'transparent', color: '#B8860B', cursor: 'pointer', fontWeight: 600,
        }}>Cancelar</button>
