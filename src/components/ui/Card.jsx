  ...
  return (
    <div
      ...
      style={{
        background: '#fff',
        borderRadius: '8px',
        boxShadow: '0 1px 4px rgba(26,18,9,0.08)',
...
export function StatCard({ label, value, sub, icon: Icon, color = '#B8860B' }) {
  return (
    <Card style={{ padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <p style={{ fontSize: '28px', fontWeight: 700, color: '#1A1209', margin: '0 0 4px', fontVariantNumeric: 'tabular-nums' }}>{value}</p>
...
export function Btn({ children, onClick, variant = 'primary', size = 'md', style = {}, disabled = false, type = 'button' }) {
  ...
  const styles = {
    primary: { background: '#B8860B', color: '#1A1209', border: '1px solid #B8860B' },
    secondary: { background: 'transparent', color: '#B8860B', border: '1px solid #B8860B' },
    danger: { background: '#C62828', color: '#fff', border: '1px solid #C62828' },
    ghost: { background: 'transparent', color: '#666', border: '1px solid #ddd' },
    dark: { background: '#1A1209', color: '#B8860B', border: '1px solid #1A1209' },
  };
